...
# CNN-XGBoost hybrid for MLGWSC-1 with proper whitening
...
class SegmentSlicer(object):
    """Slicer with built-in whitening capability."""
    ...
    def __init__(self, infile, key, step_size=0.1, peak_offset=0.6,
        ...
        logging.debug("SegmentSlicer initialized with index_step_size=%i, "
            "time_step_size=%f and segment key %s" % 
            (self.index_step_size, self.time_step_size, self.key))
        
        self.process(save_psd)
        return

    def process(self, save_psd):
        """Process (whiten) the data."""
        whitened_dss = []
        self.psds = []
        
        for ds, key in zip(self.dss, self.detectors):
            if self.white:
                # Data already whitened
                new_ds = ds[()]
            else:
                # Whiten the data
                new_ds = whiten(
        ...
        if not self.white:
            # Adjust start time after whitening (corrupted edges removed)
...
class TorchSegmentSlicer(SegmentSlicer, torch.utils.data.IterableDataset):
    """PyTorch-compatible segment slicer with whitening."""
...
def whiten(strain, delta_t=1./2048., segment_duration=0.5,
    ...
    Arguments
    ---------
    strain : numpy array or h5py dataset
    ...
    if hasattr(strain, 'shape') and not isinstance(strain, np.ndarray):
        strain = strain[()]
    ...
    if strain.ndim==1:
        ...
        white_segments = [whiten(sd_strain, delta_t=delta_t,
                ...
                low_frequency_cutoff=low_frequency_cutoff,
                psd=psd_1d, return_psd=return_psd, **kwargs) 
...
def get_clusters(triggers, cluster_threshold=0.35):
    ...
    logging.info("Clustering has resulted in %i independent triggers." % len(all_clusters))
...
def get_base_network(path=None, device=default_device, detectors=1, embedding_dim=256):
    """Return CNN feature extractor (outputs embeddings for XGBoost)."""
    network = torch.nn.Sequential(                          # Shapes
        ...
        torch.nn.Linear(1856*detectors, 64*detectors),      
        torch.nn.Dropout(p=0.5),                            
        torch.nn.ELU(),                                     
        torch.nn.Linear(64*detectors, 64*detectors),        
        torch.nn.Dropout(p=0.5),                            
        torch.nn.ELU(),
        torch.nn.Linear(64*detectors, embedding_dim)        # Output embeddings
    )
    if path is not None:
...
class CNNXGBoostNetwork(torch.nn.Module):
    """CNN-XGBoost Hybrid Network."""
    def __init__(self, cnn_network, xgb_model, regularize=False):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        embeddings = self.cnn_network(inputs)
        
        # Convert to numpy for XGBoost
        embeddings_np = embeddings.detach().cpu().numpy()
        
        # Predict with XGBoost
        proba = self.xgb_model.predict_proba(embeddings_np)
        
        # Convert back to torch tensor
        output = torch.from_numpy(proba).float().to(inputs.device)
        
        if self.regularize:
            return torch.unsqueeze(output[:, :1] - output[:, 1:], dim=1)
        else:
            return output


class ReducedStateDictCNNXGBoostNetwork(CNNXGBoostNetwork):
    """Version with reduced state dict (only saves CNN weights)."""
    def state_dict(self, *args, **kwargs):
        return self.cnn_network.state_dict(*args, **kwargs)
    
    def load_state_dict(self, *args, **kwargs):
        return self.cnn_network.load_state_dict(*args, **kwargs)


def get_coherent_network(path=None, xgb_path=None, device=default_device, 
                         detectors=1, regularize=False, reduced_state_dict=True,
                         embedding_dim=256):
    """Return an instance of a CNN-XGBoost detection network."""
    
    # Load CNN
    cnn_network = get_base_network(
        path=path, device=device, detectors=detectors, 
        embedding_dim=embedding_dim
    )
    
    # Load XGBoost model
    if xgb_path is None:
        raise ValueError("xgb_path must be provided for CNN-XGBoost network")
    ...
    if reduced_state_dict:
        network = ReducedStateDictCNNXGBoostNetwork(cnn_network, xgb_model, regularize=regularize)
    else:
        network = CNNXGBoostNetwork(cnn_network, xgb_model, regularize=regularize)
...
def get_coincident_network(path=None, xgb_path=None, device=default_device, 
                           detectors=1, regularize=False, embedding_dim=256):
    """Return an instance of a coincident CNN-XGBoost detection network."""
    
    # For coincident: separate CNN-XGBoost for each detector
    single_det_networks = []
    for _ in range(detectors):
        cnn_net = get_base_network(path=None, device=device, detectors=1, 
                                    embedding_dim=embedding_dim)
        xgb_model = xgb.XGBClassifier()
        xgb_model.load_model(xgb_path)
        network = CNNXGBoostNetwork(cnn_net, xgb_model, regularize=False)
        single_det_networks.append(network)
    
    # Load full network weights if provided
    if path is not None:
        state_dict = torch.load(path, map_location=device)
        for i, net in enumerate(single_det_networks):
            net.cnn_network.load_state_dict(state_dict)
...
def get_triggers(weights_path, xgb_path, coincident, usr, inputfile, step_size=0.1,
                 trigger_threshold=0.2, device='cpu', verbose=False, white=False,
                 whitened_file=None, low_frequency_cutoff=20., num_workers=-1):
    """Use CNN-XGBoost network to generate triggers."""
    ...
    logging.debug("Initializing CNN-XGBoost network.")
    if coincident:
        Network = get_coincident_network(
            path=weights_path, xgb_path=xgb_path,
            device=device, detectors=2, regularize=usr
        )
    else:
        Network = get_coherent_network(
            path=weights_path, xgb_path=xgb_path,
            device=device, detectors=2, regularize=usr
        )
    ...
    if num_workers > 0:
        ...
        with mp.Pool(num_workers) as pool:
            for slicer in tqdm(pool.imap_unordered(worker, arguments),
                             disable=not verbose, ascii=True, total=len(arguments)):
                slicer.stack_and_load(wdata_dict)
                triggers[slicer.key] = evaluate_slices(slicer, Network, 
...
def main():
    parser = ArgumentParser(description="CNN-XGBoost GW search script for MLGWSC-1.")

    parser.add_argument('--verbose', action='store_true', help="Print update messages.")
    parser.add_argument('--debug', action='store_true', help="Show debug messages.")
    parser.add_argument('--force', action='store_true', help="Overwrite existing output file.")

    parser.add_argument('inputfile', type=str, help="Path to input data file.")
    parser.add_argument('outputfile', type=str, help="Path to store triggers.")
    parser.add_argument('--white', action='store_true', 
                       help="Data is already whitened.")
    parser.add_argument('--softmax', action='store_true', 
                       help="Use Softmax instead of USR.")
    parser.add_argument('--coincident', action='store_true', 
                       help="Use coincident search. Default: False")
    
    parser.add_argument('-w', '--weights', type=str, default='best_cnn.pt', 
                       help="Path to CNN weights. Default: best_cnn.pt")
    parser.add_argument('--xgb-weights', type=str, default='best_xgboost.json',
                       help="Path to XGBoost model. Default: best_xgboost.json")
    
    parser.add_argument('-t', '--trigger-threshold', type=float, default=0.5, 
                       help="Threshold for triggers. Default: 0.5")
    parser.add_argument('--step-size', type=float, default=0.1, 
                       help="Sliding window step size. Default: 0.1")
    parser.add_argument('--cluster-threshold', type=float, default=0.35, 
                       help="Clustering threshold. Default: 0.35")
    parser.add_argument('--device', type=str, default='cuda', 
                       help="Device (cuda/cpu). Default: cuda")
    parser.add_argument('--debug-triggers-file', type=str, default=None)
    parser.add_argument('--debug-whitened-file', type=str, default=None)
    ...
    if args.debug_triggers_file is not None and not args.force:
        if os.path.isfile(args.debug_triggers_file):
            raise RuntimeError("Triggers file exists.")
    if args.debug_whitened_file is not None and not args.force:
    ...
    logging.info("A total of %i samples exceeded the threshold of %f" % 
                (sum([len(trigger_list) for trigger_list in triggers.values()]), 
